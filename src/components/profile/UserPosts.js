...
import { UserBlogsSkeleton } from '../skeleton';
...
export default function UserPosts() {
  ...
  return (
    <>
      {state.blog ? (
        Object.keys(state.blog).map((blogsData, _idx) => (
          <div className="bg-white dark:bg-darkMode-primary dark:text-white  dark:text-opacity-70 mb-4 p-2 w-100 max-w-5xl sm:w-full sm:p-4 h-auto sm:h-64 rounded-2xl shadow-xl hover:shadow-sm flex flex-col sm:flex-row gap-5 select-none">
            <div
              style={{
                backgroundImage: `url(${state.blog[blogsData].photo})`
              }}
            ...
            <div className="flex sm:flex-1 flex-col gap-2 p-1">
              ...
              <div className="flex gap-4 mt-auto">
                ...
                >
                  ...
                  <span>{state.blog[blogsData].likes.length}</span>
                </button>
                <button
                  type="submit"
                  className="dark:hover:bg-darkMode-orange  dark:hover:text-darkMode-base flex items-center gap-1 h-10  border border-orange-base px-3  rounded-full hover:bg-orange-base hover:text-white transition-colors focus:bg-orange-secondary focus:outline-none focus-visible:border-orange-secondary"
                >
                  ...
                  <span>{state.blog[blogsData].comments.length}</span>
                </button>
                <button
                  type="submit"
